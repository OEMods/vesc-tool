...
export class MotorConfigPage {
  ...
  async _doCompare(compareEl, statusEl) {
    ...
    compareEl.innerHTML = '';
    try {
      // Sequential, not Promise.all: requestMcConfRaw shares a single
      // pending-resolver slot on the client (_pendingMcConfRead)
      // regardless of target, so firing both reads at once means the
      // second call's resolver clobbers the first's before either
      // reply arrives — whichever packet actually comes back resolves
      // the wrong promise, and the other sits waiting for a reply
      // that's already been consumed until it times out. That's the
      // "Timed out waiting for MCCONF read" seen during Compare.
      const thisRaw = await this.client.requestMcConfRaw(null);
      const linkedRaw = await this.client.requestMcConfRaw(this.linkedCanId);
